jQuery(document).ready(function () {
	...
	projectsContainer.on('click', '.cd-slider a', function (event) {
		...
		if ($(this).parent('li').next('li').is('.current') && (mq == 'desktop')) {
			...
			singleProjectContent.load($(this).attr("href"));
		}
	});
	...
	singleProjectContent.on('click', '.close', function (event) {
		event.preventDefault();
		singleProjectContent.empty();
		singleProjectContent.removeClass('is-visible');
		singleProjectContent.append("<div class=\"lds-ripple\"><div></div><div></div></div>")
	});
	...
	$(document).keyup(function (event) {
		...
		if (event.which == '37' && intro.hasClass('projects-visible') && !(sliderNav.find('.prev').hasClass('inactive')) && (mq == 'desktop')) {
			prevSides(projectsSlider);
		} else if (event.which == '39' && intro.hasClass('projects-visible') && !(sliderNav.find('.next').hasClass('inactive')) && (mq ==
				'desktop')) {
			nextSides(projectsSlider);
	...
	function checkMQ() {
		...
		return window.getComputedStyle(document.querySelector('.cd-projects-wrapper'), '::before').getPropertyValue('content').replace(/'/g, "")
			.replace(/"/g, "");
	}
